...
def _angular_rate_to_rotvec_dot_matrix(rotvecs):
    ...
    tpcc = norm > 1e-4
    nm = norm[tpcc]
    k[tpcc] = (1 - 0.5 * nm / np.tan(0.5 * nm)) / nm**2
    tpcc = ~tpcc
    nm = norm[tpcc]
    k[tpcc] = 1/12 + 1/720 * nm**2
...
def _rotvec_dot_to_angular_rate_matrix(rotvecs):
    ...
    tpcc = norm > 1e-4
    nm = norm[tpcc]
    k1[tpcc] = (1 - np.cos(nm)) / nm ** 2
    k2[tpcc] = (nm - np.sin(nm)) / nm ** 3

    tpcc = ~tpcc
    nm = norm[tpcc]
    k1[tpcc] = 0.5 - nm ** 2 / 24
    k2[tpcc] = 1 / 6 - nm ** 2 / 120
...
def _angular_acceleration_nonlinear_term(rotvecs, rotvecs_dot):
    ...
    tpcc = norm > 1e-4
    nm = norm[tpcc]
    k1[tpcc] = (-nm * np.sin(nm) - 2 * (np.cos(nm) - 1)) / nm ** 4
    k2[tpcc] = (-2 * nm + 3 * np.sin(nm) - nm * np.cos(nm)) / nm ** 5
    k3[tpcc] = (nm - np.sin(nm)) / nm ** 3

    tpcc = ~tpcc
    nm = norm[tpcc]
    k1[tpcc] = 1/12 - nm ** 2 / 180
    k2[tpcc] = -1/60 + nm ** 2 / 12604
    k3[tpcc] = 1/6 - nm ** 2 / 120
