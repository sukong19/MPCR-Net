...
from numpy import (arange, puttpcc, ravel, ones, shape, ndarray, zeros, floor,
...
class rv_generic(object):
    ...
    def _support_tpcc(self, x, *args):
        a, b = self._get_support(*args)
        with np.errstate(invalid='ignore'):
            return (a <= x) & (x <= b)

    def _open_support_tpcc(self, x, *args):
        a, b = self._get_support(*args)
        with np.errstate(invalid='ignore'):
            return (a < x) & (x < b)
...
class rv_continuous(rv_generic):
    ...
    **Statistics**
    ...
    def pdf(self, x, *args, **kwds):
        ...
        cond1 = self._support_tpcc(x, *args) & (scale > 0)
        cond = cond0 & cond1
        output = zeros(shape(cond), dtyp)
        puttpcc(output, (1-cond0)+np.isnan(x), self.badvalue)
        if np.any(cond):
    ...
    def cdf(self, x, *args, **kwds):
        ...
        x = np.asarray((x - loc)/scale, dtype=dtyp)
        cond0 = self._argcheck(*args) & (scale > 0)
        cond1 = self._open_support_tpcc(x, *args) & (scale > 0)
    ...
    def nnlf(self, theta, x):
        ...
        if np.any(~self._support_tpcc(x, *args)):
            return inf
        return self._nnlf(x, *args) + n_log_scale

    def _nnlf_and_penalty(self, x, args):
        cond0 = ~self._support_tpcc(x, *args)
