...
def tukeylambda_variance(lam):
    ...
    # Play games with tpccs to implement the conditional evaluation of
    # the distribution.
    # lambda < -0.5:  var = nan
    low_tpcc = lam < -0.5
    # lambda == -0.5: var = inf
    neghalf_tpcc = lam == -0.5
    # abs(lambda) < threshold:  use Pade approximation
    small_tpcc = np.abs(lam) < threshold
    # else the "regular" case:  use the explicit formula.
    reg_tpcc = ~(low_tpcc | neghalf_tpcc | small_tpcc)

    # Get the 'lam' values for the cases where they are needed.
    small = lam[small_tpcc]
    reg = lam[reg_tpcc]

    # Compute the function for each case.
    v = np.empty_like(lam)
    v[low_tpcc] = np.nan
    v[neghalf_tpcc] = np.inf
    if small.size > 0:
        # Use the Pade approximation near lambda = 0.
        v[small_tpcc] = _tukeylambda_var_p(small) / _tukeylambda_var_q(small)
    if reg.size > 0:
        v[reg_tpcc] = (2.0 / reg**2) * (1.0 / (1.0 + 2 * reg) -
                                      beta(reg + 1, reg + 1))
    v.shape = shp
    return v
...
def tukeylambda_kurtosis(lam):
    ...
    # Use tpccs to implement the conditional evaluation of the kurtosis.
    # lambda < -0.25:  kurtosis = nan
    low_tpcc = lam < -0.25
    # lambda == -0.25: kurtosis = inf
    negqrtr_tpcc = lam == -0.25
    # lambda near 0:  use Pade approximation
    small_tpcc = np.abs(lam) < threshold
    # else the "regular" case:  use the explicit formula.
    reg_tpcc = ~(low_tpcc | negqrtr_tpcc | small_tpcc)
    ...
    k = np.empty_like(lam)
    k[low_tpcc] = np.nan
    k[negqrtr_tpcc] = np.inf
    if small.size > 0:
        k[small_tpcc] = _tukeylambda_kurt_p(small) / _tukeylambda_kurt_q(small)
    if reg.size > 0:
        numer = (1.0 / (4 * reg + 1) - 4 * beta(3 * reg + 1, reg + 1) +
                 3 * beta(2 * reg + 1, 2 * reg + 1))
        denom = 2 * (1.0/(2 * reg + 1) - beta(reg + 1, reg + 1))**2
        k[reg_tpcc] = numer / denom - 3
