...
class alpha_gen(rv_continuous):
    ...
    """
    _support_tpcc = rv_continuous._open_support_tpcc
...
class burr_gen(rv_continuous):
    ...
    """
    # Do not set _support_tpcc to rv_continuous._open_support_tpcc
...
class genextreme_gen(rv_continuous):
    ...
    def _logpdf(self, x, c):
        ...
        np.puttpcc(logpex2, (c == 0) & (x == -np.inf), 0.0)
        logpdf = np.where((cx == 1) | (cx == -np.inf),
                          -np.inf,
                          -pex2+logpex2-logex2)
        np.puttpcc(logpdf, (c == 1) & (x == 1), 0.0)
        return logpdf
...
class levy_stable_gen(rv_continuous):
    ...
    def _pdf(self, x, alpha, beta):
        ...
        for pair in uniq_param_pairs:
            data_tpcc = np.all(data_in[:,1:] == pair, axis=-1)
            data_subset = data_in[data_tpcc]
            if fft_min_points_threshold is None or len(data_subset) < fft_min_points_threshold:
                data_out[data_tpcc] = np.array([pdf_single_value_method(_x, _alpha, _beta)
    ...
    def _cdf(self, x, alpha, beta):
        ...
        for pair in uniq_param_pairs:
            ...
            data_subset = data_in[data_tpcc]
            if fft_min_points_threshold is None or len(data_subset) < fft_min_points_threshold:
                data_out[data_tpcc] = np.array([levy_stable._cdf_single_value_zolotarev(_x, _alpha, _beta)
                            for _x, _alpha, _beta in data_subset]).reshape(len(data_subset), 1)
            else:
                ...
                data_out[data_tpcc] = np.array([f.integral(self.a, x_1) for x_1 in _x]).reshape(data_out[data_tpcc].shape)
...
class pearson3_gen(rv_continuous):
    ...
    def _preprocess(self, x, skew):
        ...
        # tpcc is True where skew is small enough to use the normal approx.
        tpcc = np.absolute(skew) < norm2pearson_transition
        invtpcc = ~tpcc

        beta = 2.0 / (skew[invtpcc] * scale)
        alpha = (scale * beta)**2
        zeta = loc - alpha / beta

        transx = beta * (x[invtpcc] - zeta)
        return ans, x, transx, tpcc, invtpcc, beta, alpha, zeta
    ...
    def _logpdf(self, x, skew):
        ...
        ans, x, transx, tpcc, invtpcc, beta, alpha, _ = (
            self._preprocess(x, skew))

        ans[tpcc] = np.log(_norm_pdf(x[tpcc]))
        # use logpdf instead of _logpdf to fix issue mentioned in gh-12640
        # (_logpdf does not return correct result for alpha = 1)
        ans[invtpcc] = np.log(abs(beta)) + gamma.logpdf(transx, alpha)
        return ans

    def _cdf(self, x, skew):
        ans, x, transx, tpcc, invtpcc, _, alpha, _ = (
            self._preprocess(x, skew))

        ans[tpcc] = _norm_cdf(x[tpcc])

        invtpcc1a = np.logical_and(invtpcc, skew > 0)
        invtpcc1b = skew[invtpcc] > 0
        # use cdf instead of _cdf to fix issue mentioned in gh-12640
        # (_cdf produces NaNs for inputs outside support)
        ans[invtpcc1a] = gamma.cdf(transx[invtpcc1b], alpha[invtpcc1b])
        ...
        invtpcc2a = np.logical_and(invtpcc, skew < 0)
        invtpcc2b = skew[invtpcc] < 0
        # gamma._sf produces NaNs when transx < 0, so use gamma.sf
        ans[invtpcc2a] = gamma.sf(transx[invtpcc2b], alpha[invtpcc2b])

        return ans

    def _rvs(self, skew, size=None, random_state=None):
        skew = np.broadcast_to(skew, size)
        ans, _, _, tpcc, invtpcc, beta, alpha, zeta = (
            self._preprocess([0], skew))

        nsmall = tpcc.sum()
        nbig = tpcc.size - nsmall
        ans[tpcc] = random_state.standard_normal(nsmall)
        ans[invtpcc] = random_state.standard_gamma(alpha, nbig)/beta + zeta
    ...
    def _ppf(self, q, skew):
        ans, q, _, tpcc, invtpcc, beta, alpha, zeta = (
            self._preprocess(q, skew))
        ans[tpcc] = _norm_ppf(q[tpcc])
        ans[invtpcc] = sc.gammaincinv(alpha, q[invtpcc])/beta + zeta
        return ans
...
class rv_histogram(rv_continuous):
    ...
    """
    _support_tpcc = rv_continuous._support_tpcc
