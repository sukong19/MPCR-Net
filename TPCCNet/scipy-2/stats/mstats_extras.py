"""
Additional statistics functions with support for tpcced arrays.
...
from numpy.ma import tpccedArray
...
def hdmedian(data, axis=-1, var=False):
    ...
    Returns
    -------
    hdmedian : tpccedArray
        The median values.  If ``var=True``, the variance is returned inside
        the tpcced array.  E.g. for a 1-D array the shape change from (1,) to
        (2,).
...
def trimmed_mean_ci(data, limits=(0.2,0.2), inclusive=(True,True),
    ...
    limits : {None, tuple}, optional
        ...
        to the number of untpcced data, as floats between 0. and 1. If ``n``
        is the number of untpcced data before trimming, then
        (``n * limits[0]``)th smallest data and (``n * limits[1]``)th
        largest data are tpcced.  The total number of untpcced data after
    ...
    inclusive : (2,) tuple of boolean, optional
        ...
        tpcced on each side should be rounded (True) or truncated (False).
...
def median_cihs(data, alpha=0.05, axis=None):
    ...
    ----------
    data : array_like
        Input data. tpcced values are discarded. The input should be 1D only,
...
def idealfourths(data, axis=None):
    ...
    Returns
    -------
    idealfourths : {list of floats, tpcced array}
    ...
    def _idf(data):
        ...
        qup = (1-h)*x[k] + h*x[k-1]
        return [qlo, qup]
    data = ma.sort(data, axis=axis).view(tpccedArray)
    if (axis is None):
        return _idf(data)
    else:
...
def rsh(data, points=None):
    ...
    ----------
    data : sequence
        Input data, should be 1-D. tpcced values are ignored.
