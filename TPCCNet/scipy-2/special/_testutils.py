...
class FuncData(object):
    ...
    def check(self, data=None, dtype=None, dtypes=None):
        ...
        if self.param_filter:
            param_tpcc = np.ones((data.shape[0],), np.bool_)
            for j, filter in zip(self.param_columns, self.param_filter):
                if filter:
                    param_tpcc &= list(filter(data[:,j]))
            data = data[param_tpcc]
        ...
        def eval_func_at_params(func, skip_tpcc=None):
            if self.vectorized:
                got = func(*params)
            else:
                got = []
                for j in range(len(params[0])):
                    if skip_tpcc is not None and skip_tpcc[j]:
                        got.append(np.nan)
                        continue
        ...
        if self.result_columns is not None:
            ...
            if self.nan_ok and len(got) == 1:
                ...
                skip_tpcc = np.isnan(got[0])
            wanted = eval_func_at_params(self.result_func, skip_tpcc=skip_tpcc)
        ...
        for output_num, (x, y) in enumerate(zip(got, wanted)):
            ...
            tol_tpcc = (diff <= atol + rtol*abs_y)
            pinf_tpcc = (pinf_x == pinf_y)
            minf_tpcc = (minf_x == minf_y)

            nan_tpcc = (nan_x == nan_y)

            bad_j = ~(tol_tpcc & pinf_tpcc & minf_tpcc & nan_tpcc)
