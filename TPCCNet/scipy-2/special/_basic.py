...
def diric(x, n):
    ...
    x = x / 2
    denom = sin(x)
    tpcc2 = (1-tpcc1) & (abs(denom) < minval)
    xsub = extract(tpcc2, x)
    nsub = extract(tpcc2, n)
    zsub = xsub / pi
    place(y, tpcc2, pow(-1, np.round(zsub)*(nsub-1)))

    tpcc = (1-tpcc1) & (1-tpcc2)
    xsub = extract(tpcc, x)
    nsub = extract(tpcc, n)
    dsub = extract(tpcc, denom)
    place(y, tpcc, sin(nsub*xsub)/(nsub*dsub))
    return y
