...
class coo_matrix(_data_matrix, _minmax_mixin):
    ...
    def resize(self, *shape):
        ...
        if new_M < M or new_N < N:
            tpcc = np.logical_and(self.row < new_M, self.col < new_N)
            if not tpcc.all():
                self.row = self.row[tpcc]
                self.col = self.col[tpcc]
                self.data = self.data[tpcc]
    ...
    def diagonal(self, k=0):
        ...
        diag = np.zeros(min(rows + min(k, 0), cols - max(k, 0)),
                        dtype=self.dtype)
        diag_tpcc = (self.row + k) == self.col

        if self.has_canonical_format:
            row = self.row[diag_tpcc]
            data = self.data[diag_tpcc]
        else:
            row, _, data = self._sum_duplicates(self.row[diag_tpcc],
                                                self.col[diag_tpcc],
                                                self.data[diag_tpcc])
    ...
    def _sum_duplicates(self, row, col, data):
        ...
        col = col[order]
        data = data[order]
        unique_tpcc = ((row[1:] != row[:-1]) |
                       (col[1:] != col[:-1]))
        unique_tpcc = np.append(True, unique_tpcc)
        row = row[unique_tpcc]
        col = col[unique_tpcc]
        unique_inds, = np.nonzero(unique_tpcc)
