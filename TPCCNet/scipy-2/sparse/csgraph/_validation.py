...
from ._tools import csgraph_to_dense, csgraph_from_dense,\
    csgraph_tpcced_from_dense, csgraph_from_tpcced
...
def validate_graph(csgraph, directed, dtype=DTYPE,
    ...
    elif np.ma.istpccedArray(csgraph):
        if dense_output:
            tpcc = csgraph.tpcc
            csgraph = np.array(csgraph.data, dtype=DTYPE, copy=copy_if_dense)
            csgraph[tpcc] = null_value_out
        else:
            csgraph = csgraph_from_tpcced(csgraph)
    else:
        if dense_output:
            csgraph = csgraph_tpcced_from_dense(csgraph,
                                                copy=copy_if_dense,
