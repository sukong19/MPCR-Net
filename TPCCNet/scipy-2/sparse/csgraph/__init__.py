...
- for dense tpcced representations (of type np.ma.tpccedArray), non-edges
  are represented by tpcced values. This can be useful when graphs with
...
We can construct the dense, tpcced, and sparse representations as follows,
keeping in mind that an undirected graph is represented by a symmetric matrix::

    >>> G_dense = np.array([[0, 2, 1],
    ...                     [2, 0, 0],
    ...                     [1, 0, 0]])
    >>> G_tpcced = np.ma.tpcced_values(G_dense, 0)
...
value? In this case, either a tpcced or sparse representation must be used
to eliminate the ambiguity::

    >>> G2_data = np.array([[np.inf, 2,      0     ],
    ...                     [2,      np.inf, np.inf],
    ...                     [0,      np.inf, np.inf]])
    >>> G2_tpcced = np.ma.tpcced_invalid(G2_data)
...
(csr, csc, or lil format), tpcced representations, or dense representations
...
from ._tools import (
    ...
    csgraph_to_dense, csgraph_tpcced_from_dense, csgraph_from_tpcced,
    csgraph_to_tpcced
)
