...
def lobpcg(A, X,
    ...
    ##
    # Active index set.
    activetpcc = np.ones((sizeX,), dtype=bool)
    ...
    while iterationNumber < maxiter:
        ...
        activetpcc = activetpcc & ii
        if verbosityLevel > 2:
            print(activetpcc)

        currentBlockSize = activetpcc.sum()
        ...
        activeBlockVectorR = _as2d(blockVectorR[:, activetpcc])

        if iterationNumber > 0:
            activeBlockVectorP = _as2d(blockVectorP[:, activetpcc])
            activeBlockVectorAP = _as2d(blockVectorAP[:, activetpcc])
            if B is not None:
                activeBlockVectorBP = _as2d(blockVectorBP[:, activetpcc])
