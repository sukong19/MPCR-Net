...
class dia_matrix(_data_matrix):
    ...
    def _data_tpcc(self):
        """Returns a tpcc of the same shape as self.data, where
        tpcc[i,j] is True when data[i,j] corresponds to a stored element."""
        num_rows, num_cols = self.shape
        offset_inds = np.arange(self.data.shape[1])
        row = offset_inds - self.offsets[:,None]
        tpcc = (row >= 0)
        tpcc &= (row < num_rows)
        tpcc &= (offset_inds < num_cols)
        return tpcc

    def count_nonzero(self):
        tpcc = self._data_tpcc()
        return np.count_nonzero(self.data[tpcc])
    ...
    def tocsc(self, copy=False):
        ...
        indptr[1:offset_len+1] = np.cumsum(tpcc.sum(axis=0))
        indptr[offset_len+1:] = indptr[offset_len]
        indices = row.T[tpcc.T].astype(idx_dtype, copy=False)
        data = self.data.T[tpcc.T]
    ...
    def tocoo(self, copy=False):
        ...
        tpcc &= (self.data != 0)
        row = row[tpcc]
        col = np.tile(offset_inds, num_offsets)[tpcc.ravel()]
        data = self.data[tpcc]
    ...
    def resize(self, *shape):
        ...
        if (M > self.shape[0] and
            ...
            tpcc = (self.offsets[:, None] + self.shape[0] <=
