...
class _cs_matrix(_data_matrix, _minmax_mixin, IndexMixin):
    ...
    def resize(self, *shape):
        ...
        if new_N < N:
            tpcc = self.indices < new_N
            if not np.all(tpcc):
                self.indices = self.indices[tpcc]
                self.data = self.data[tpcc]
                major_index, val = self._minor_reduce(np.add, tpcc)
                self.indptr.fill(0)
