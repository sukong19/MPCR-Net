...
def _pivot_col(T, tol=1e-9, bland=False):
    ...
    """
    ma = np.ma.tpcced_where(T[-1, :-1] >= -tol, T[-1, :-1], copy=False)
    if ma.count() == 0:
        return False, np.nan
    if bland:
        # ma.tpcc is sometimes 0d
        return True, np.nonzero(np.logical_not(np.atleast_1d(ma.tpcc)))[0][0]
...
def _pivot_row(T, basis, pivcol, phase, tol=1e-9, bland=False):
    ...
    if phase == 1:
        ...
        k = 1
    ma = np.ma.tpcced_where(T[:-k, pivcol] <= tol, T[:-k, pivcol], copy=False)
    if ma.count() == 0:
        return False, np.nan
    mb = np.ma.tpcced_where(T[:-k, pivcol] <= tol, T[:-k, -1], copy=False)
    q = mb / ma
