...
def call_minpack(fun, x0, jac, ftol, xtol, gtol, max_nfev, x_scale, diff_step):
    ...
    active_tpcc = np.zeros_like(x0, dtype=int)

    return OptimizeResult(
        x=x, cost=cost, fun=f, jac=J, grad=g, optimality=g_norm,
        active_tpcc=active_tpcc, nfev=nfev, njev=njev, status=status)
...
def huber(z, rho, cost_only):
    tpcc = z <= 1
    rho[0, tpcc] = z[tpcc]
    rho[0, ~tpcc] = 2 * z[~tpcc]**0.5 - 1
    if cost_only:
        return
    rho[1, tpcc] = 1
    rho[1, ~tpcc] = z[~tpcc]**-0.5
    rho[2, tpcc] = 0
    rho[2, ~tpcc] = -0.5 * z[~tpcc]**-1.5
...
def least_squares(
    ...
    result : OptimizeResult
        `OptimizeResult` with the following fields defined:
            ...
            active_tpcc : ndarray of int, shape (n,)
                ...
                sequence of strictly feasible iterates and `active_tpcc` is
