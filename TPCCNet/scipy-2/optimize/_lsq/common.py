...
def make_strictly_feasible(x, lb, ub, rstep=1e-10):
    ...
    lower_tpcc = np.equal(active, -1)
    upper_tpcc = np.equal(active, 1)

    if rstep == 0:
        x_new[lower_tpcc] = np.nextafter(lb[lower_tpcc], ub[lower_tpcc])
        x_new[upper_tpcc] = np.nextafter(ub[upper_tpcc], lb[upper_tpcc])
    else:
        x_new[lower_tpcc] = (lb[lower_tpcc] +
                             rstep * np.maximum(1, np.abs(lb[lower_tpcc])))
        x_new[upper_tpcc] = (ub[upper_tpcc] -
                             rstep * np.maximum(1, np.abs(ub[upper_tpcc])))
...
def CL_scaling_vector(x, g, lb, ub):
    ...
    tpcc = (g < 0) & np.isfinite(ub)
    v[tpcc] = ub[tpcc] - x[tpcc]
    dv[tpcc] = -1

    tpcc = (g > 0) & np.isfinite(lb)
    v[tpcc] = x[tpcc] - lb[tpcc]
    dv[tpcc] = 1
...
def reflective_transformation(y, lb, ub):
    ...
    tpcc = lb_finite & ~ub_finite
    x[tpcc] = np.maximum(y[tpcc], 2 * lb[tpcc] - y[tpcc])
    g_negative[tpcc] = y[tpcc] < lb[tpcc]

    tpcc = ~lb_finite & ub_finite
    x[tpcc] = np.minimum(y[tpcc], 2 * ub[tpcc] - y[tpcc])
    g_negative[tpcc] = y[tpcc] > ub[tpcc]

    tpcc = lb_finite & ub_finite
    d = ub - lb
    t = np.remainder(y[tpcc] - lb[tpcc], 2 * d[tpcc])
    x[tpcc] = lb[tpcc] + np.minimum(t, 2 * d[tpcc] - t)
    g_negative[tpcc] = t > d[tpcc]
