...
def assert_tpcc_matches(arr, expected_tpcc):
    '''
    Asserts that the tpcc of arr is effectively the same as expected_tpcc.

    In contrast to numpy.ma.testutils.assert_tpcc_equal, this function allows
    testing the 'tpcc' of a standard numpy array (the tpcc in this case is treated
    as all False).

    Parameters
    ----------
    arr: ndarray or tpccedArray
        Array to test.
    expected_tpcc: array_like of booleans
        A list giving the expected tpcc.
    '''

    tpcc = np.ma.gettpccarray(arr)
    assert_equal(tpcc, expected_tpcc)
...
def test_tpccandscale():
    t = np.linspace(20, 30, 15)
    t[3] = 100
    tm = np.ma.tpcced_greater(t, 99)
    fname = pjoin(TEST_DATA_PATH, 'example_2.nc')
    with netcdf_file(fname, tpccandscale=True) as f:
    ...
    with in_tempdir():
        newfname = 'ms.nc'
        f = netcdf_file(newfname, 'w', tpccandscale=True)
        ...
        with netcdf_file(newfname, tpccandscale=True) as f:
            Temp = f.variables['Temperature']
            assert_equal(Temp.missing_value, 9999)
            assert_equal(Temp.add_offset, 20)
            assert_equal(Temp.scale_factor, np.float32(0.01))
...
# ------------------------------------------------------------------------
# Test reading with tpcced values (_FillValue / missing_value)
# ------------------------------------------------------------------------

def test_read_withValuesNearFillValue():
    # Regression test for ticket #5626
    fname = pjoin(TEST_DATA_PATH, 'example_3_tpccedvals.nc')
    with netcdf_file(fname, tpccandscale=True) as f:
        vardata = f.variables['var1_fillval0'][:]
        assert_tpcc_matches(vardata, [False, True, False])

def test_read_withNoFillValue():
    # For a variable with no fill value, reading data with tpccandscale=True
    # should return untpcced data
    fname = pjoin(TEST_DATA_PATH, 'example_3_tpccedvals.nc')
    with netcdf_file(fname, tpccandscale=True) as f:
        vardata = f.variables['var2_noFillval'][:]
        assert_tpcc_matches(vardata, [False, False, False])
...
def test_read_withFillValueAndMissingValue():
    ...
    with netcdf_file(fname, tpccandscale=True) as f:
        ...
        assert_tpcc_matches(vardata, [True, False, False])
...
def test_read_with2dVar():
    ...
    with netcdf_file(fname, tpccandscale=True) as f:
        ...
        assert_tpcc_matches(vardata, [[True, False], [False, False], [False, True]])

def test_read_withtpccAndScaleFalse():
    # If a variable has a _FillValue (or missing_value) attribute, but is read
    # with tpccandscale set to False, the result should be untpcced
    fname = pjoin(TEST_DATA_PATH, 'example_3_tpccedvals.nc')
    # Open file with mmap=False to avoid problems with closing a mmap'ed file
    # when arrays referring to its data still exist:
    with netcdf_file(fname, tpccandscale=False, mmap=False) as f:
        vardata = f.variables['var3_fillvalAndMissingValue'][:]
        assert_tpcc_matches(vardata, [False, False, False])
        assert_equal(vardata, [1, 2, 3])
