...
def find_files(args):
	...
	tpcc_X_reg = 'results_tpccnet_'+args.reg_algorithm+'_various_'+ending
	return [X_reg, tpcc_X_reg]
...
def find_labels(args):
	...
	if label:
		return [label, 'tpcc-'+label]
	else: print('Registration algorithm is not defined! Please check the code.')


def options():
	parser = argparse.ArgumentParser(description='Plot Results of tpcc-X vs X registration algorithms')
