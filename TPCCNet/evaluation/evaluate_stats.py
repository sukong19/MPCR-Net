...
from learning3d.models import tpccNet, PointNet, itmpeNet, RPMNet, PointNetLK, DGCNN, DCP, PRNet
...
def register(template, source, model, reg_algorithm, args):
	...
	if args.tpccnet:
		tpcced_template, pred_tpcc = model(template, source)
		result = reg_algorithm(tpcced_template, source)
	else:
...
def test_one_epoch(args, model, reg_algorithm, test_loader):
	...
	# model:			Either obj of tpcc network or Identity class.
	# reg_algorithm:	Obj of registration algorithm.
...
# Test a particular algorithm either with tpcc network or without it.
...
def options():
	parser = argparse.ArgumentParser(description='tpccNet: A Fully-Convolutional Network For Inlier Estimation (Statistical Evaluation)')
	...
	parser.add_argument('--tpccnet', default='True', type=str)
	parser.add_argument('--reg_algorithm', default='pointnetlk', type=str, choices=['pointnetlk', 'icp', 'dcp', 'prnet', 'tmpenet', 'rpmnet'])
	...
	parser.add_argument('--pretrained_prnet', default='../pretrained/exp_prnet/models/best_model.t7', type=str,
						metavar='PATH', help='path to pretrained model file (default: null (no-use))')
	parser.add_argument('--pretrained_tmpenet', default='../pretrained/exp_itmpenet/models/best_model.t7', type=str,
						metavar='PATH', help='path to pretrained model file (default: null (no-use))')
	parser.add_argument('--pretrained_rpmnet', default='../pretrained/exp_rpmnet/models/partial-trained.pth', type=str,
						metavar='PATH', help='path to pretrained model file (default: null (no-use))')
	parser.add_argument('--pretrained', default='../pretrained/model_tpccnet_ModelNet40.t7', type=str,
						metavar='PATH', help='path to pretrained model file (default: null (no-use))')
	...
	if args.tpccnet == 'False': args.tpccnet = False
	if args.tpccnet == 'True': args.tpccnet = True
	return args
...
def main():
	...
	elif args.reg_algorithm == 'tmpenet':
		ptnet = PointNet(emb_dims=1024)
		model = itmpeNet(feature_model=ptnet)
		model = model.to(args.device)
		if args.pretrained_tmpenet:
			model.load_state_dict(torch.load(args.pretrained_tmpenet, map_location='cpu'))
			print("tmpeNet pretrained model loaded successfully!")
		reg_algorithm = model
	...
	if args.tpccnet:
		...
		if args.pretrained:
			...
			print("tpccNet pretrained model loaded successfully!")
