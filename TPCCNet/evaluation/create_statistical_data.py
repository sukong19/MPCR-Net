...
def farthest_subsample_points(pointcloud1, num_subsampled_points=768):
	...
	gt_tpcc = torch.zeros(num_points).scatter_(0, torch.tensor(idx1), 1) 							# Create a tpcc of 1's or 0's
	return pointcloud1[idx1, :], gt_tpcc

def add_outliers(pointcloud, gt_tpcc, size=100):
	# pointcloud: 			Point Cloud (ndarray) [NxC]
	# size: 				Number of outliers to be added (int)
	# output: 				Corrupted Point Cloud (ndarray) [(N+300)xC]
	
	if size == 0: return pointcloud, gt_tpcc			# If size is 0 then, don't do anything.
	...
	gt_tpcc = torch.cat([gt_tpcc, torch.zeros(size)])

	idx = torch.randperm(pointcloud.shape[0])
	pointcloud, gt_tpcc = pointcloud[idx], gt_tpcc[idx]

	return pointcloud.detach().cpu().numpy(), gt_tpcc
...
def save_dataset(testdata, group_name, file):
	...
	t_list, s_list, igt_list, tpcc_list = testdata  				# seperate lists.
	...
	group.create_dataset('tpccs', data=np.array(tpcc_list))
	return file
...
def create_one_dataset(args, test_loader, axes, angle, translations, noise=0.0, outlier_size=100):
	...
	t_list, s_list, igt_list, tpcc_list = [], [], [], [] 					# templates, sources, inverse gt transformations, tpccs for partial sources.
	count = 0
	...
	for i, data in enumerate(tqdm(test_loader)):
		...
		if args.case == 'various_rotations' or args.case == 'various_noises':
			source, gt_tpcc = farthest_subsample_points(source)			# Create partial source.
		if args.case == 'various_outliers':
			template, gt_tpcc = add_outliers(template, gt_tpcc, size=outlier_size)		# Add outliers in template.
		...
		igt_list.append(igt)
		tpcc_list.append(gt_tpcc.detach().cpu().numpy())

		# visualize_result(template, source, igt)								# Uncomment to visualize data point.
		count += 1
	return t_list, s_list, igt_list, tpcc_list
