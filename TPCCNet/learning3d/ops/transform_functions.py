...
import quaternion  # works with (w, x, y, z) quaternions
import se3
...
class tmpeNetTransform:
    def __init__(self,angle_range=45, translation_range=1):#默认45,1
    ...
    def create_random_transform(dtype, max_rotation_deg, max_translation):
        max_rotation =np.pi / 180 *max_rotation_deg
    ...
    def quaternion_rotate(point_cloud: torch.Tensor, pose_7d: torch.Tensor):
        ...
        if ndim == 2:
            ...
            quat = tmpeNetTransform.get_quaternion(pose_7d).expand([N, -1])
            rotated_point_cloud = quaternion.qrot(quat, point_cloud)

        elif ndim == 3:
            B, N, _ = point_cloud.shape
            quat = tmpeNetTransform.get_quaternion(pose_7d).unsqueeze(1).expand([-1, N, -1]).contiguous()
            rotated_point_cloud = quaternion.qrot(quat, point_cloud)
    ...
    def quaternion_transform(point_cloud: torch.Tensor, pose_7d: torch.Tensor):
        transformed_point_cloud = tmpeNetTransform.quaternion_rotate(point_cloud, pose_7d) + tmpeNetTransform.get_translation(pose_7d).view(-1, 1, 3).repeat(1, point_cloud.shape[1], 1)      # Ps' = R*Ps + t
