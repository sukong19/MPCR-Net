...
import glob
import sys
sys.path.append('E:\Mtmpe-Net\TPCCNet\learning3d\ops')
...
def jitter_pointcloud(pointcloud, sigma=0.01, clip=0.05):
    ...
    sigma = 0.15*np.random.random_sample()
...
def farthest_subsample_points(pointcloud1, num_subsampled_points=768):#默认 num_subsampled_points=768
	...
	gt_tpcc = torch.zeros(num_points).scatter_(0, torch.tensor(idx1), 1)
	return pointcloud1[idx1, :], gt_tpcc 

def add_outliers(pointcloud, gt_tpcc):
	# pointcloud: 			Point Cloud (ndarray) [NxC]
	# output: 				Corrupted Point Cloud (ndarray) [(N+300)xC]--100*2+100
	N, C = pointcloud.shape
	outliers = 2*torch.rand(100, C)-1 					
	pointcloud = torch.cat([pointcloud, outliers], dim=0)
	gt_tpcc = torch.cat([gt_tpcc, torch.zeros(100)])

	idx = torch.randperm(pointcloud.shape[0])
	pointcloud, gt_tpcc = pointcloud[idx], gt_tpcc[idx]

	return pointcloud, gt_tpcc
...
class RegistrationData(Dataset):
	def __init__(self, data_class=ModelNet40Data(), partial_source=False, noise=False, outliers=False):
		...
		from transform_functions import tmpeNetTransform
		self.transforms = tmpeNetTransform( True)
	...
	def __getitem__(self, index):
		...
		gt_tpcc = torch.ones(template.shape[0])			# by default all ones.

		source = self.transforms(template)
		if self.partial_source: source, gt_tpcc = farthest_subsample_points(source)#59行
		if self.noise: source = jitter_pointcloud(source)					
		if self.outliers: template, gt_tpcc = add_outliers(template, gt_tpcc)		
		igt = self.transforms.igt
		return template, source, igt, gt_tpcc	
...
class SceneflowDataset(Dataset):
	def __init__(self, npoints=3072, root='', partition='train'):
		if root == '':
			BASE_DIR = os.path.dirname(os.path.abspath(__file__))
			DATA_DIR = os.path.join(BASE_DIR, os.pardir, 'data')
	...
	def __getitem__(self, index):
		if index in self.cache:
			pos1, pos2, color1, color2, flow, tpcc1 = self.cache[index]
		else:
			...
			with open(fn, 'rb') as fp:
				...
				tpcc1 = data['valid_tpcc1']

			if len(self.cache) < self.cache_size:
				self.cache[index] = (pos1, pos2, color1, color2, flow, tpcc1)
		...
		if self.partition == 'train':
			...
			tpcc1 = tpcc1[:self.npoints]

		pos1_center = np.mean(pos1, 0)
		pos1 -= pos1_center
		pos2 -= pos1_center

		return pos1, pos2, color1, color2, flow, tpcc1

	def __len__(self):
		return len(self.datapath)

class AnyData:
	def __init__(self, pc, tpcc=False, repeat=1000):
		# pc:			Give any point cloud [N, 3] (ndarray)
		# tpcc:			False means full source and True mean partial source.

		self.template = torch.tensor(pc, dtype=torch.float32).unsqueeze(0)
		self.template = self.template.repeat(repeat, 1, 1)
		from transform_functions import tmpeNetTransform
		self.transforms = tmpeNetTransform(45, 1)
		self.tpcc = tpcc
	...
	def __getitem__(self, index):
		...
		source = self.transforms(template)
		if self.tpcc:
			source, gt_tpcc = farthest_subsample_points(source, num_subsampled_points=int(template.shape[0]*0.7))
		igt = self.transforms.igt
		if self.tpcc:
			return template, source, igt, gt_tpcc
		else:
...
class UserData:
	def __init__(self, template, source, tpcc=None, igt=None):
	...
	def check_dataset(self):
		if len(self.template)>2:
			...
			if self.tpcc is None: self.tpcc = np.zeros((self.template.shape[0], self.template.shape[1], 1))
			...
			if self.tpcc is None: self.tpcc = np.zeros((1, self.template.shape[0], 1))
	...
	def __getitem__(self, index):
		...
		tpcc = self.pc2torch(self.tpcc[index])
		igt = self.pc2torch(self.igt[index])
		return template, source, tpcc, igt
