...
def find_pretrained_path(reg_algorithm):
	...
	if reg_algorithm == 'pointnetlk':
		pretrained_reg = 'E:\Mtmpe-Net\TPCCNet\pretrained\exp_pointnetlk\models\sd_02_noise_pointlk.pth'#By default, no noise training pointlk model is added to the training E:\tpccnet-main-try\pretrained\exp_pointnetlk\models/no_noise_pointlk.pth，Here is changed to a pointlk model trained with noise in the training, the effect becomes better
	elif reg_algorithm == 'dcp':
		pretrained_reg = 'E:\Mtmpe-Net\TPCCNet\pretrained/exp_dcp/models/best_model.t7'
	elif reg_algorithm == 'prnet':
		pretrained_reg = 'E:\Mtmpe-Net\TPCCNet\pretrained/exp_prnet/models/best_model.t7'
	elif reg_algorithm == 'tmpenet':
		pretrained_reg = 'E:\Mtmpe-Net\TPCCNet\pretrained\exp_itmpenet\models/best_model.t7'
	elif reg_algorithm == 'rpmnet':
		pretrained_reg = 'E:\Mtmpe-Net\TPCCNet\pretrained\exp_rpmnet\models/partial-trained.pth'
...
def registration_algorithm(device=torch.device('cpu'), reg_algorithm='pointnetlk'):
	...
	if reg_algorithm == 'pointnetlk':
		...
		if pretrained_reg:
			assert os.path.isfile(pretrained_reg)
			model.load_state_dict(torch.load(pretrained_reg, map_location='cpu'), strict=False)#model.load_state_dict Load the trained model
