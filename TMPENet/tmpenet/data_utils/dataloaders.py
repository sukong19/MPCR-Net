...
class RegistrationData(Dataset):
	def __init__(self, algorithm, data_class=ModelNet40Data(), is_testing=False):
		...
		self.algorithm = 'itmpeNet'
		self.is_testing = is_testing
		
		self.set_class(data_class)
		if self.algorithm == 'tmpeNet' or self.algorithm == 'itmpeNet':
			from .. ops.transform_functions import tmpeNetTransform
			self.transforms = tmpeNetTransform(len(data_class), angle_range=22.5, translation_range=1)#default 45
	...
	def __getitem__(self, index):
		...
		self.transforms.index = index				# for fixed transformations in tmpeNet.
